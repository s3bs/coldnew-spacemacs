...
In this configuration, the =user-emacs-directory= always refers to the emacs
configuration's init.el parent directory.

#+BEGIN_SRC emacs-lisp
  ;; We set the `user-emacs-directory' here so we can use a command-line
  ;; argument to switch between different emacs configurations like following:
  ;;
  ;;    emacs -q -l ~/coldnew-spacemacs/init.el
...
The main package management in my emacs is [[https://github.com/cask/cask][Cask]], which is a really nice package
manager like npm, cargo ...etc.

[[https://github.com/cask/cask][Cask]] can also install packages according to your emacs version, so don't be
afraid if you get version conflicts after upgrade emacs.

Take my emacs as example, after initializing [[https://github.com/cask/cask][Cask]], all packages installed by
=package.el= are just saved to the =.cask= folder according to your emacs version.
...
Since we already installed pallet via cask, we just need to use the following code
...
*** Enable async package compilation

Allow `async' to compile your packages asynchronously. You can also define
if you want to include all packages or select specific ones.

#+BEGIN_SRC emacs-lisp
  (require 'async)
  (async-bytecomp-package-mode 1)
  (customize-set-variable 'async-bytecomp-allowed '(all))
#+END_SRC


** quelpa

[[https://github.com/quelpa/quelpa][quelpa]] is another package manager which can help you to build an ELPA compatible
...
To start loading packages in right order, we need to add the following as the last
thing in our emacs config.
...
Following are my methods to add directories to load-path ~recursively~, this
function also creates a directory, if the directory does not exist.

If you don't have any local elisp and all packages are mantained by cask, elpa or
spacemacs, you can skip the following code.
...
[[https://github.com/syl20bnr/spacemacs][Spacemacs]] is an emacs starterkit focusing on [[https://gitorious.org/evil/pages/Home][Evil]], which emulates the vim
keymap on Emacs.

I put my emacs settings on top of spacemacs and set it to use emacs (=holy=)
keybindings.

In my config file, the original =~/.spacemacs= file has moved to
=~/.emacs.d/spacemacs.d/init.el= , I also advice spacemacs function to
prevent orphan packages being deleted by spacemacs. If you want the vim keymap, change
the appropriate variable in the spacemacs init.

After all spacemacs init is done, switch back to the =*scratch*= buffer.

** Load basic spacemacs configuration file

The latest spacemacs can setup =SPACEMACSDIR= to load customize spacemacs init.el
file.
...
The original spacemacs suggests to clone it to =~/.emacs.d=, I really do not like
...
#+BEGIN_SRC emacs-lisp
  ...
  (let* ((spacemacs-dir
         ...
         ;; following line is a current HOTFIX to allow spacemacs set package repos
...
*** Enable/Disable global highlight by default

Only if you want though. I like a highlighted line.
Spacemacs enables this one already.
...
I don't like the =ido-mode=. Or maybe I do. #Enabled for now.
...
here, some are not set or just kept for backward compability.
...
#+BEGIN_SRC emacs-lisp
  ...
  (customize-set-variable 'mac-pass-command-to-system nil)
#+END_SRC

** Don't ask me when I close emacs with a process running
...
** Don't ask me when I kill a process buffer
...
Most of my config is written in this file. There is no need to track the emacs's
custom-setting.
...
#+BEGIN_SRC emacs-lisp
  (when (require 'async nil 'noerror)
    ;; If I edit my init.org, async generate init.el when save.
    (defun tangle-init ()
...
Just let me use =y= or =n= with no =RET= required and I'm quite happy.
...
Ignore case when completing names filename etc

#+BEGIN_SRC emacs-lisp
  (setq completion-ignore-case t
        read-file-name-completion-ignore-case t)
#+END_SRC
** Basic find and dired settings
...
Always make sure that =*scratch*= is in lisp-interaction-mode
...
Since the current Emacs default runs on UTF-8, there's no need to setup the encoding.

For languages, though Traditional Chinese is my mothertone, I still prefer use
=en_US= to display time info.
#TODO trailing what happens if i don't have this
#+BEGIN_SRC emacs-lisp :tangle no
...
Some buildin packages are not loaded by emacs, load it here.
...
** htmlize                                                  :htmlize:review:

Disabled for now. Don't think i use it
...
** ascii-art-to-unicode                                             :review:disabled:
...
#+BEGIN_SRC emacs-lisp :noweb yes
  ...
  (add-hook 'auto-save-hook 'my-desktop-auto-save)
  (add-hook 'after-save-hook 'my-desktop-auto-save)
  (add-hook 'find-file-hook 'my-desktop-auto-save)


  (defvar my-desktop-session-dir
    (concat (getenv "HOME") "/.emacs.d/.cache/desktop-sessions/")
  ...
  (defun my-desktop-kill-emacs-hook ()
    ...
    (when (file-exists-p (concat my-desktop-session-dir "last-session-" (format-time-string "%Y-%j")))
      (setq desktop-file-modtime
            (nth 5 (file-attributes (desktop-full-file-name (concat my-desktop-session-dir "last-session-" (format-time-string "%Y-%j")))))))
    (my-desktop-save (concat "last-session-" (format-time-string "%Y-%j"))))
...
~This might be already provided by spacemacs~
...
[[https://julien.danjou.info/projects/emacs-packages][rainbow-mode]] is a minor mode for Emacs which displays strings representing colors
...
[[https://github.com/steckerhalter/discover-my-major][discover-my-major]] makes you discover key bindings and their meaning for the
...
[[https://github.com/ShingoFukuyama/manage-minor-mode][manage-minor-mode]] lets you manage your minor-mode in a dedicated
buffer.
...
Let's you decorate functions dynamically.
...
verify-url is a little tool that is used to find out invalid urls in the buffer or
region.
...
*** Force emacs to always save buffers (even if file is not modified)
...
I only sets the executable permission for the  =owener=, but doesn't change it for group or
others user.
...
*** Duplication current line

#+BEGIN_SRC emacs-lisp
  (defun duplicate-line ()
    "Clone line at cursor, leaving the latter intact."
    ;; This version doesn't mess around with the kill ring
    (interactive "*")
    (save-excursion
...
;; previous implementation used recursion but occationally ran into recursion limit
(defun move-line-interactive ()
    (let ((disabled nil))
      (while (not disabled)
        (setq disabled (_move-line-interactive) ))))

  (defun _move-line-interactive ()
    (let ((user-event (read-event "<up> <down> to move line and <return> to finish")))
      (pcase user-event
        (`up (move-line-up) nil)
        (`return t)
        (`down (move-line-down) nil)
        (_ nil))))
#+END_SRC
** Debug
...
A really nice command to help me to find errors on elisp buffer.
...
I always use a dark theme for coding. I'm using the zenburn theme that comes with
spacemacs.

However, you can always add your own customization. I.e. the
emacs theme called =coldnew-theme-night= and =coldnew-theme-day=, you can find
them at [[file:theme/coldnew-theme.el]].

Before the use of emacs's =load-theme= function, I advise it to fully
unload the previous theme before loading a new one.
...
** Setup keybindings                                       :keybinding:todo:
...
have many lines, it's not recommand to enable linum-mode.
...
I'm leaving this here for reference, but I don't like emacs.
So if you want some vim additional vim commands ...
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; tramp and projectile might not play well together
...
#+BEGIN_SRC emacs-lisp
  ;; change default naming of buffers with same file to uniqify them
...
Sometimes I'll clean the =*scratch*= buffer by killing it. Add the following function
...
I prefer to use =dired+= mode instead of plain old dired
#+BEGIN_SRC emacs-lisp
  (req-package dired+)
#+END_SRC
...
Setting fringe margin. Default is 8. Setting it to 0 removes IMHO useful arrows
that are visual aids when lines are too long.
...
#+BEGIN_SRC emacs-lisp
  (req-package helm
    ...
    (progn
      ...
      ;; set helm so it can select anything even not match
...
** Graphviz                                                       :graphviz:
TODO there is currently a bug in graphviz and i don't really need it.
#+BEGIN_SRC emacs-lisp :tangle no
...
*** Highlight functions or macros belone to cl.el
    ...
    TODO - fails compiling at the moment
#+BEGIN_SRC emacs-lisp :tangle no
...
** Ediff support for revision history

Use ediff to show the difference between the current revision and a
previous one.
...
Uses `vc.el' or `rcs.el' depending on `ediff-version-control-package'."
    ...
    (let ((file (or (buffer-file-name)
      ...
      (setq rev1 (read-string
                  (format "Revision to compare (default latest revision): ")))
...
#+BEGIN_SRC emacs-lisp :noweb yes :results silent
  ...
  (eval-after-load 'ediff '(setq ediff-window-setup-function #'ediff-setup-windows-plain))

  ;; Rebind 'C-x v = from vc-diff to ediff-current-buffer-revision
  (global-set-key "\C-xv=" 'ediff-current-buffer-revision)
#+END_SRC
...
*** Use git-timemachine to browse the file history
...
Old style emacs backups if you don't have git or want an additional backup
just in case.

#+BEGIN_SRC emacs-lisp
  ;; Emacs creates a backup only when you save the first time. This is incompatible
  ;; with most people’s basic editing behavior of leaving Emacs and its buffers
  ;; open all the time and constantly making changes and writing them to disk.
  (setq
   make-backup-files t      ; although this should be on by default
   backup-by-copying t      ; don't clobber symlinks
   backup-directory-alist '(("." . "~/.emacs.d/cache/saves"))    ; don't litter my fs tree
   delete-old-versions t
   kept-new-versions 10
   kept-old-versions 1
   vc-make-backup-files t   ; make backups of versioned files too
   version-control t)       ; use version'ed backups
...
I'm putting those here until I know better.

#+BEGIN_SRC emacs-lisp
  ; although might switch to helm-find mhh
  (global-set-key [f5] 'find-name-dired)
  ; helm-mini is much better suited than helm-buffer-list
  ; which has an annoying history subbuffer than can contain non existent buffers
  (global-set-key (kbd "C-x b") 'helm-mini)

  ; enable use of colored ibuffer-list-buffers
...
Have not decided yet how much I'll adapt ==coldemacs==
